...
At this point, you've implemented the code to build a fully [BOLT 3](https://github.com/lightning/bolts/blob/master/03-transactions.md) compliant Lightning funding and commitment transactions! You're part of a very small group of people in the world who have done this.

Since we're playing the part of Alice, you've now successfully built the **Funding Transaction** and Alice's version of the **Commitment Transaction**. In the "real world", we would have also build Bob's version locally so that we can generate a signature to send to him, but we'll leave that as a bonus exercise to the ambitious learner.
...
Okay, by now you're probably itching to learn about **payments**. After all, Lightning is a meant to help Bitcoin realize the dream outlined in the whitepaper: a Peer-to-Peer Electronic Cash System.

Below is a diagram that we saw earlier when discussing how we can create new public keys for each commitment state. Specifically, this diagram does a good job of showing *which keys* we'll use in each commitment state. For example, if you zoom in below, you'll see that, in **Comittment State 2**:
...
We'll, let's answer questions 1 - 3. If you know the answer to question 4, please let me know!

To answer questions 1 - 3, it's easiest if we jump straight into the deep and and start with how to route payments across the Lightning Network. While it may seem counterintuitive to start with the more complex example, it's actually easier. This is because the **protocol** that defines how payments are sent is the same *regardless of if you're sending a payment to a direct peer or across multiple channels*. **In other words, the protocol (rules of the game) will be the same regardless of if Alice is simply paying Bob (a direct peer) OR if Alice is paying Dianne, who is connected to Bob (Alice -> Bob -> Dianne).**
...
Let's begin our journey into Lightning payments by reviewing how things word at a high level, starting with the end-result in mind. Routing a payment across the Lightning network just means that the **channel balance distributions** will change for each channel, such that, ***for every channel pair***, when the payment is complete:
...
- **Bob Has More Bitcoin**: Before the payment, Bob had a cliam to 2,500,500 sats **across both channels**. After the payment, Bob has a claim to 2,505,500 sats **across both channels**. He now has 5,000 more sats.
...
The 5,000 sats are **fees** paid to Bob! Alice will have to pay fees to each hop along the payment route to incentivize the them to forward the payment. Without this economic incentize, forwarding nodes (Bob, in this case) have little to gain (outside of altruism) when forwarding the payment.
