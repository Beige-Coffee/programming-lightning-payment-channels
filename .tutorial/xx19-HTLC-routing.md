...
Another caveat: there are actually a few more protocol messages that are involved with making payments! We'll review them shortly. As a brief teaser, if you recall, we'll have to provide the **Per-Commitment Secret** each time we update our channel so that our counterparty can punish us if we attempt to cheat in the future. We'll review those protocol messages shortly.
...
Instead of failing an HTLC by claiming it on-chain, which would defeat the entire purpose of Lightning, peers will send either an `update_fail_htlc` or `update_fail_malformed_htlc` message to the counterparty that **sent** them the HTLC. These messages serve similar purposes - to inform the sender why the HTLC failed.
