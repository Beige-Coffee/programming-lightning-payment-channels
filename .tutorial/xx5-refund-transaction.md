...
If Bob becomes unresponsive or stops cooperating with Alice, then Alice won't be able to retrieve her funds from the funding transaction! Therefore, she needs to have a backup plan. This "backup plan" is often referred to as the capability to "exit" an off-chain system. Ideally, Alice (or any user) should be able to "unilaterally exit", meaning that they do not need another party's cooperation to claim their funds on-chain.
