...
To successfully complete this function, you'll have to use the secp256k1 context to calculate the **Revocation Public Key** using the formula we reviewed above (also provided below), which is provided in [BOLT 3](https://github.com/lightning/bolts/blob/master/03-transactions.md#revocationpubkey-derivation).
...
This is very similar to the previous exercise. That said, the notable difference is that we'll operate on secrets (scalars) instead of public keys (points).
