...
First, let's confirm that our Funding Transaction is still unspent. This will reinforce the concept that all of the transactions we just created during this course were off-chain.

Enter this command within your **Shell**, ensuring to replace the `funding_tx_id` with your **Funding Tx ID**. The `0` after it just indicates that the funding input was at index 0 of this transaction, which is guaranteed for this course but is not always true.
```
bitcoin-cli gettxout funding_tx_id 0
```
...
```
bitcoin-cli sendrawtransaction htlc_tx_hex
```
...
```
bitcoin-cli sendrawtransaction htlc_timeout_tx_hex
```
