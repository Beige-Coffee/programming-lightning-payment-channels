...
To successfully complete this function, you'll need to produce a **Per-Commitment Secret** (returned as `[u8; 32]`) for a given `commitment_number`. The result must be in accordance with the specifications outlined in BOLT 3.
...
To complete this exercise successfully, you'll need to implement the following algorithm in Rust.
...
Below are a few Rust tips that will help you in your journey:
1. Start by initializing `P`, a **mutable copy** of the seed. You'll need to do this because the algorithm needs to be able to update `P`.
2. Next, you'll need to check if the bit at the provided index is SET in the commitment number. You can do this by using a bitwise AND operation in Rust: `if commitment_number & (1 << bit_position) == (1 << bit_position)`
3. If the bit is SET, you'll need to flip the corresponding bit in **P**. You can flip the corresponding bit in **P** by using the following: `p[bit_position / 8] ^= 1 << (bit_position & 7)`.
4. After flipping the bit, perform a SHA256 hash operation on the result to get the new **P**: `p = Sha256::hash(&p).to_byte_array()`.
...
The expression `commitment_number & (1 << bit_position)` checks if the bit at position `bit_position` is set to 1.
...
We calculate which byte contains our bit (`bit_position / 8`) and which bit within that byte (`bit_position & 7`), then use XOR to flip it.
...
Also, make sure to serialize each public key using `.serialize()`. Then, we can feed those bytes into our hash engine with `.input()`.
...
Now, let's finalize the hash and convert the result into a public key. To be clear, we're treating a hash output as a secret key, which we then use to derive its public key.
