...
4. **Attach the witness to the transaction**
   - You can input the witness into the transaction using `signed_tx.input[input_index].witness = witness`.
...
Now that our witness is defined, let's attach it to the appropriate input in the transaction. To do this, we'll specify the index of the input we signed.
