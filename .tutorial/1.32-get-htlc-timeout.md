...
- `finalize_htlc_timeout`: This generates our (Alice's) signature using Alice's **derived HTLC private key** and adds it to the witness (along with Bob's **derived HTLC private key** signature and the witness script), resulting in a fully signed HTLC Timeout Transaction, which can only be broadcast and mined *after* the timelock expires.
