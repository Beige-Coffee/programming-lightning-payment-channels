...
use crate::types::{ChannelKeys, Bolt3TestVector, Bolt3Htlc, HtlcDirection};
...
pub fn build_complete_commitment_transaction(
    ...
    // Trim dust HTLCs
    let offered_trimmed: Vec<_> = offered_htlcs.iter()
        .filter(|(amt, _)| !is_htlc_dust(*amt, dust_limit_satoshis, feerate_per_kw))
        .cloned()
        .collect();
    
    let received_trimmed: Vec<_> = received_htlcs.iter()
...
pub fn build_bolt3_commitment_with_htlcs(
    ...
    println!("\n=== HTLC Keys Debug ===");
    println!("Expected local_htlcpubkey:  {}", hex::encode(test_vector.local_htlcpubkey.serialize()));
    println!("Actual local_htlc_key:      {}", hex::encode(commitment_keys.local_htlc_key.serialize()));
    println!("Expected remote_htlcpubkey: {}", hex::encode(test_vector.remote_htlcpubkey.serialize()));
    println!("Actual remote_htlc_key:     {}", hex::encode(commitment_keys.remote_htlc_key.serialize()));
